...
if (isset($_POST['submit'])) {
    ...
    $pword = md5($_POST['password']); // Hash the password

    // Include the database connection file
    include("database.php");

    // Prepare the SQL query to update the user
    $sql = "UPDATE users SET firstname='$updated_firstname', lastname='$updated_lastname', gmail='$updated_gmail', password='$pword' WHERE id='$id'";

    // Execute the SQL query
    if (mysqli_query($conn, $sql)) {
        // Redirect to a success page or display a success message
        header("Location: /access/user_id/index.php?user_id=$id");
