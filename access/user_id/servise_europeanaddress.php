...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Retrieve the user information from the form
    $convertedamount = $_POST['address'];
    $id = $_GET['id'];
    
    // Prepare the SQL query to insert the user data into the table
    $sql = "INSERT INTO europeanaddress (address,UserID) 
            VALUES (?, ?)";
    
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("si", $convertedamount,$id);        
...
<head>
    ...
    <title>European Address Order</title>
    <style>
        body {
        ...
        body::before {
            content: "";
            background-image: url("/access/img/europeanaddress.png");
    ...
    <div class="container">
        <form method="post" action="" id="multiStepForm" enctype="multipart/form-data">
            ...
            <div class="step" id="step2">
                <h2>Step 1: Address Details</h2>
                <input type="text" id="address" name="address" placeholder="Address That You Want To Confirm" required>
